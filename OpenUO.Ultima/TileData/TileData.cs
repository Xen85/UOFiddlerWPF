            ...
            if (!string.IsNullOrEmpty(filePath))
            {
                using (FileStream fs = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    ...
                    for (int i = 0; i < 0x4000; ++i)
                    {
                        ...
                        m_LandData[i] = new LandData(ReadNameString(bin), flags,i);
                    }

                    m_ItemData = new ItemData[21200];
                    m_HeightTable = new int[m_ItemData.Length];
                    ...
                    for (int i = 0; i < m_ItemData.Length; ++i)
                    {
                        ...
                        m_ItemData[i] = new ItemData(ReadNameString(bin), flags, weight, quality, quantity, value, height, anim,i);
